...
INT multi_init(EQUIPMENT *pequipment) {
   ...
   if (m_info->num_channels_input) {
      ...
      /* Unit */
      status = db_find_key(hDB, m_info->hKeyRoot, "Settings/Unit Input", &hKey);
      if (status == DB_NO_KEY) {
         int n = m_info->num_channels_input;
         char *unit = (char *)calloc(n, 32);
         db_create_key(hDB, m_info->hKeyRoot, "Settings/Unit Input", TID_STRING);
         db_find_key(hDB, m_info->hKeyRoot, "Settings/Unit Input", &hKey);
         db_set_data(hDB, hKey, unit, n*32, n, TID_STRING);
         free(unit);
      }

      /* Format */
      status = db_find_key(hDB, m_info->hKeyRoot, "Settings/Format Input", &hKey);
      if (status == DB_NO_KEY) {
         int n = m_info->num_channels_input;
         char *unit = (char *)calloc(n, 32);
         for (int i=0 ; i<n ; i++)
            strlcpy(unit + i*32, "%f2", 32);
         db_create_key(hDB, m_info->hKeyRoot, "Settings/Format Input", TID_STRING);
         db_find_key(hDB, m_info->hKeyRoot, "Settings/Format Input", &hKey);
         db_set_data(hDB, hKey, unit, n*32, n, TID_STRING);
         free(unit);
      }
   }
   ...
   if (m_info->num_channels_output) {
      ...
      /* Unit */
      status = db_find_key(hDB, m_info->hKeyRoot, "Settings/Unit Output", &hKey);
      if (status == DB_NO_KEY) {
         int n = m_info->num_channels_input;
         char *unit = (char *)calloc(n, 32);
         db_create_key(hDB, m_info->hKeyRoot, "Settings/Unit Output", TID_STRING);
         db_find_key(hDB, m_info->hKeyRoot, "Settings/Unit Output", &hKey);
         db_set_data(hDB, hKey, unit, n*32, n, TID_STRING);
         free(unit);
      }

      /* Format */
      status = db_find_key(hDB, m_info->hKeyRoot, "Settings/Format Output", &hKey);
      if (status == DB_NO_KEY) {
         int n = m_info->num_channels_input;
         char *format = (char *)calloc(n, 32);
         for (int ii=0 ; ii<n ; ii++)
            strlcpy(format + ii*32, "%f2", 32);
         db_create_key(hDB, m_info->hKeyRoot, "Settings/Format Output", TID_STRING);
         db_find_key(hDB, m_info->hKeyRoot, "Settings/Format Output", &hKey);
