...
public:
   mdevice(std::string eq_name, std::string dev_name,
      ...
      if (equipment[idx].name[0] == 0) {
         char str[256];
         snprintf(str, sizeof(str), "Equipment \"%s\" not found in equipment table", eq_name.c_str());
      ...
      if (mEq->driver == nullptr) { // create new device driver list
         ...
         for (n = 0; mEq->driver[n].name[0]; n++) {
            if (mEq->driver[n].name == dev_name) {
               char str[256];
               snprintf(str, sizeof(str), "Device \"%s\" defined twice for equipment \"%s\"", dev_name.c_str(),
                       eq_name.c_str());
               cm_msg(MERROR, "device::device", "%s", str);
               mthrow(str);
               return;
                   ...
                   double factor = std::nan(""), double offset = std::nan(""))
   {
      ...
      if (mEq->driver[mDevIndex].flags & DF_INPUT) {
         ...
         mOdbSettings["Names Input"][chn_index].set_string_size(name, 32);
         ...
         mOdbSettings["Names Output"][chn_index].set_string_size(name, 32);
         mName.push_back(name);
         ...
         mOdbSettings["Names"][chn_index].set_string_size(name, 32);
         mName.push_back(name);
      }
   ...
   void define_varf(std::string name = "", double threshold = std::nan(""),
                   std::string formula = "", std::string unit = "")
   {
      int chn_index = 0;
      ...
      if (mEq->driver[mDevIndex].flags & DF_INPUT) {
         ...
         mOdbSettings["Update Threshold"][chn_index] = (float) threshold;
         mOdbSettings["Input Formula"][chn_index].set_string_size(formula, FORMULA_SIZE);
         mOdbSettings["Input Unit"][chn_index].set_string_size(unit, 32);
         mOdbSettings["Names Input"][chn_index].set_string_size(name, 32);
         mName.push_back(name);
         ...
         mOdbSettings["Output Formula"][chn_index] = formula;
         mOdbSettings["Input Unit"][chn_index].set_string_size(unit, 32);
         mOdbSettings["Names Output"][chn_index].set_string_size(name, 32);
         mName.push_back(name);
