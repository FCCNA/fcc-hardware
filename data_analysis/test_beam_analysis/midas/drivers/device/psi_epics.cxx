...
INT psi_epics_init(HNDLE hKey, void **pinfo, INT channels)
{
   ...
   *pinfo = info;
   ...
   for (i = 0; i < channels; i++) {
      ...
      /* connect command channel for separator and slits */
      if (info->device_type[i] == DT_SEPARATOR || info->device_type[i] == DT_SLIT) {
...
INT psi_epics_set(CA_INFO * info, INT channel, float value)
{
   ...
   if (info->demand[channel].chan_id) {
      if (info->device_type[channel] == DT_BEAMBLOCKER)
      ...
      else if (info->device_type[channel] == DT_SLIT) {
         ...
         status = ca_put(DBR_STRING, info->extra[channel].chan_id, "MOVE_TO_SOL");
         if (status != CA_M_SUCCESS)
            cm_msg(MERROR, "psi_epics_set", "Cannot write to EPICS channel %s", info->extra[channel].name);
      }
