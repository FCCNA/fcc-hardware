...
INT frontend_init()
{
   /* enable/disable certain command in device driver */

   // default number of channels is set in the epics_driver struct
   // at the top of this file. If ODB has more channels, we do not want
   // them truncated. Set number of channels to what we see in ODB. K.O.

   int status;
   HNDLE hkey;
   std::string odb_path;
   odb_path += "/Equipment/";
   odb_path += equipment[0].name;
   odb_path += "/Variables/Measured";
   
   status = db_find_key(hDB, 0, odb_path.c_str(), &hkey);
   if (hkey) {
      KEY key;
      status = db_get_key(hDB, hkey, &key);
      if (status == DB_SUCCESS) {
	 int default_values = epics_driver[0].channels;

	 //printf("key [%s] size %d default %d\n", key.name, key.num_values, default_values);
	 
	 if (key.num_values > default_values) {
	    epics_driver[0].channels = key.num_values;
	 }
      }
   }
...
INT frontend_loop()
{
  ...
  /* ss_sleep(50); */
  status = cm_yield(50); /* 15Feb05 */
  
  //if (status != SS_TIMEOUT)
  //  cm_msg(MERROR, "frontend_loop", "cm_yield status %d", status);
