...
/*---- Function declarations ---------------------------------------*/
   ...
   INT ss_shm_flush(const char *name, const void *shm_adr, size_t shm_size, HNDLE handle, bool wait_for_thread);
   ...
   /*---- socket routines ----*/
   INT EXPRT ss_socket_connect_tcp(const char* hostname, int tcp_port, int* sockp, std::string* error_msg_p);
   INT EXPRT ss_socket_listen_tcp(bool listen_localhost, int tcp_port, int* sockp, int* tcp_port_p, std::string* error_msg_p);
   INT EXPRT ss_socket_close(int* sockp);
   INT EXPRT ss_socket_wait(int sock, int millisec);
   INT EXPRT ss_socket_get_peer_name(int sock, std::string* host, int* port);
