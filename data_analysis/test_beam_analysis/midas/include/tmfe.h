...
class TMFeRpcHandlerInterface
{
 public:
   ...
   virtual TMFeResult HandleBinaryRpc(const char* cmd, const char* args, std::vector<char>& result) { return TMFeOk(); };
};
...
public:
   ...
   //void Msg(int message_type, const char *filename, int line, const char *routine, const char *message); // ambigious with format Msg()
   void Msg(int message_type, const char *filename, int line, const char *routine, const std::string& message);
