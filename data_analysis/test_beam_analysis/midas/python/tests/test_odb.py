...
class LocaleContextManager():
    """
    Find the first available locale that uses "," as the
    decimal separator, so we can test that our ODB access
    works even if the default float format looks like 
    "1,23e5" rather than "1.23e5".

    Use like:

    ```
    with LocaleContextManager() as context:
        some_test()
    ```
    """
    def __enter__(self):
        try:
            # No good way to get a list of locales that host system
            # supports without calling out to "locale -a"...
            locales = str(subprocess.run(["locale", "-a"], capture_output=True).stdout, "UTF-8").splitlines()
...
class TestOdb(unittest.TestCase):
    ...
    def set_and_readback(self, path, value, expected_key_type, force_type=False):
        if force_type:
            self.client.odb_set(path, value, explicit_new_midas_type=expected_key_type)
        else:
    ...
    def set_and_readback_from_parent_dir(self, parent_path, key_name, value, expected_key_type, force_type=False):
        child_path = parent_path + "/" + key_name

        if force_type:
            self.client.odb_set(child_path, value, explicit_new_midas_type=expected_key_type)
        else:
    ...
    def change_locale_then_set_and_readback_from_parent_dir(self, parent_path, key_name, value, expected_key_type):
        with LocaleContextManager() as locale_context:
            self.set_and_readback_from_parent_dir(parent_path, key_name, value, expected_key_type)
    ...
    def testInt64(self):
        self.set_and_readback("/pytest/int64", -1, midas.TID_INT64, True)
        self.set_and_readback("/pytest/int64", 1, midas.TID_INT64, True)
        self.set_and_readback("/pytest/int64", [1230000000000000, 4, -36], midas.TID_INT64, True)
        self.set_and_readback("/pytest/int64", [4500000000000000], midas.TID_INT64, True)
        self.set_and_readback("/pytest/int64", [ctypes.c_int64(-23), ctypes.c_int64(14700000000000000)], midas.TID_INT64)
        ...
        self.set_and_readback_from_parent_dir("/pytest", "int64_2", [123, -45, 40000000000000000], midas.TID_INT64, True)
        self.set_and_readback_from_parent_dir("/pytest", "int64_2", 37, midas.TID_INT64, True)
        
    def testQword(self):
        self.set_and_readback("/pytest/uint64", [ctypes.c_uint64(78900000000000000), ctypes.c_uint64(135)], midas.TID_QWORD)
        self.set_and_readback("/pytest/uint64", ctypes.c_uint64(123), midas.TID_QWORD)
        self.set_and_readback("/pytest/uint64", ctypes.c_uint64(45600000000000000), midas.TID_QWORD)
        self.set_and_readback("/pytest/uint64", (ctypes.c_uint64 * 5)(*[1,2,3,4,500000000000000000]), midas.TID_QWORD)
    ...
    def testFloat(self):
        ...
        self.change_locale_then_set_and_readback_from_parent_dir("/pytest", "float2", [ctypes.c_float(1.23), ctypes.c_float(3.45)], midas.TID_FLOAT)
        self.change_locale_then_set_and_readback_from_parent_dir("/pytest", "float2", ctypes.c_float(3.5), midas.TID_FLOAT)
    ...
    def testDouble(self):
        ...
        self.change_locale_then_set_and_readback_from_parent_dir("/pytest", "double2", (ctypes.c_double(9.11), ctypes.c_double(3.14)), midas.TID_DOUBLE)
        self.change_locale_then_set_and_readback_from_parent_dir("/pytest", "double2", ctypes.c_double(3.14), midas.TID_DOUBLE)
