...
function dlgCenter(dlg) {
   ...
   // allow for scrolling of very high dialog boxes
...
function dlgShow(dlg, modal, param) {
   ...
   if (d.childNodes === undefined) {
      dlgAlert("Dialog '" + dlg + "' has no title bar");
      return;
   }
...
function dlgGeneral(p) {
   /* general dialog containing p.html code and other optional parameters
      p.iddiv          - the name of the dialog div (optional)
      p.width/p.height - minimal width/height of dialog (optional)
      p.x/p.y          - initial position of dialog (optional)
   */
   
   // First make sure you removed existing iddiv
   if (document.getElementById(p.iddiv)) document.getElementById(p.iddiv).remove();
   let d = document.createElement("div");
   d.className = "dlgFrame";
   if (p.iddiv) d.id = p.iddiv;
   d.style.zIndex = "30";
   d.style.overflow = "hidden";
   d.style.resize = "both";
   d.style.width = p.width? p.width + "px" : "400px";
   d.style.height = p.height ? p.height + "px" : "200px";
   d.style.minWidth = p.minWidth ? p.minWidth + "px" : d.style.width;
   d.style.minHeight = p.minHeight ? p.minHeight + "px" : d.style.height;
   ...
   dlgTitle.innerText = p.iddiv ? p.iddiv + " dialog" : "General dialog";
   ...
   d.style.height = (content.scrollHeight + dlgTitle.offsetHeight + 5 ) + "px";
   d.style.width = (content.scrollWidth + 5 ) + "px";
   ...
   // Resize observer to watch for dialog resizing
...
function dlgQuery(string, value, queryCallback, param, size) {
   let d = document.createElement("div");
   d.className = "dlgFrame";
   d.style.zIndex = "31";
   ...
   d.innerHTML = "<div class=\"dlgTitlebar\" id=\"dlgMessageTitle\">&nbsp;</div>" +
       ...
       "<div id=\"dlgMessageString\">" + string + "&nbsp;&nbsp;<input type='text' size='"+size+"' id='dlgQueryInput' onkeydown='return dlgQueryKeyDown(event, this);' value='" + value + "'></input></div>" +
       "<br /><br />" +
