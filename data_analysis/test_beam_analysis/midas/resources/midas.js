...
async function mjsonrpc_send_request(req)
{
   ...
   return new Promise(function(resolve, reject) {
      ...
      if (req == "send invalid json")
         ...
         // special serialization for BigInt numbers
         let j = JSON.stringify(req, (key, value) =>
...
function db_get_value(path, callback) {
   ...
   mjsonrpc_call("db_get_values", req).then(rpc => callback(rpc))
...
function bkToObj(array) {
   ...
   ///   .array[]       <-- array of converted data (UINT16, UINT32, UINT32, INT32, FLOAT, DOUBLE)
   ///
   ...
   while(i8 < e.data_size + event_header_size) {
      ...
      if (b.type === TID_INT8)
         ...
         b.array = new BigInt64Array(b.data);
      else if (b.type === TID_UINT64)
         b.array = new BigUint64Array(b.data);
      else if (b.type === TID_FLOAT)
