         ...
         <tr>
            ...
            <td style="text-align: left">
               <select id="odbCreateType"
                  ...
                  <option value=17>Long Integer (64-bit)
                  <option value=18>Quad Word (64-bit)
...
function odb_browser(id, path, picker) {
   ...
   td.innerHTML =
      ...
      '<img src="icons/folder-open.svg" title="Load ODB Ctrl+O" ' +
      'onclick="odb_load(this)">&nbsp;&nbsp;' +
      '<img src="icons/save.svg" title="Save ODBn Ctrl+S" ' +
      'onclick="odb_save_picker(this)">&nbsp;&nbsp;' +
      '<img src="icons/download.svg" title="Export ODB" ' +
      'onclick="odb_export(this)">&nbsp;&nbsp;' +
      '<img src="icons/upload.svg" title="Import ODB" ' +
      'onclick="odb_import(this)">&nbsp;&nbsp;' +
      '<img src="icons/search.svg" title="Search keys" ' +
      'onclick="search_key(this)">&nbsp;&nbsp;' +
      '<img src="icons/trash-2.svg" title="Delete keys Ctrl+Delete" ' +
   ...
   tr.childNodes[7].innerHTML +=
      "&nbsp;<span title='Hide key details' style='display:inline;float:right'>" +
      "<a id='expRight' href='#' onclick='expand(this);return false;'>&#x21E4;</a>"+
      "</span>";
   tr.childNodes[7].style.width = '65px';
...
function odb_picker(path, callback, param) {
   ...
   odb_browser('dlgOdbPicker', path, true);
...
function odb_setoption(p, index) {
   ...
   // remove any selected to force update when the ODB value changes later
   if (p.innerHTML.includes('selected=""'))
      p.innerHTML = p.innerHTML.replace('selected=""', '');
   if (p.innerHTML.includes('selected'))
      p.innerHTML = p.innerHTML.replace('selected=""', '');
   ...
   if (odb.picker) {
      ...
      if (event.shiftKey && !headerRow) {
         ...
         if (i1 === tb.childNodes.length)
            i1 = 4; // none selected, so use first one
...
function context_menu(event) {
   // don't show context menu if we are in input box (user wants traditional copy/paste)
   if (event.target.tagName === 'INPUT')
      return;
...
function expand(e) {
   ...
   let table = document.getElementById('odbTable');
...
let _odb_path;
function odb_save_picker(e) {
   let tb = getOdbTb(e);
   _odb_path = tb.odb.path;
   file_picker("", "*.json", odb_save, true);
}
function odb_save(filename) {
   mjsonrpc_db_copy([_odb_path]).then(rpc => {
      ...
      file_save_ascii(filename, odbJson, "ODB subtree \"" + _odb_path +
                      "\" saved to file \"" + filename + "\"");

   }).catch(error => mjsonrpc_error_alert(error));
}

async function odb_import(e) {
   try {
      // Check if showOpenFilePicker is available in the browser
      // The "else" is more of a JS standard
      if (window.showOpenFilePicker) {
         let fileHandle;
         [fileHandle] = await window.showOpenFilePicker();
         const file = await fileHandle.getFile();
         const text = await file.text();
         pasteBuffer(e, text, file.name);
      } else {
         // Show a traditional file input element 
         ...
         fileInput.addEventListener('change', (event) => {
            ...
            if (file) {
               ...
               reader.onload = async (evObj) => {
                  const text = evObj.target.result;
                  pasteBuffer(e, text, file.name);
               };
...
function odb_load(e) {
   ...
   let path = tb.odb.path;
   _odb_path = tb.odb.path;
   file_picker("", "*.json", loadFile);
}

function loadFile(filename) {
   file_load_ascii(filename, (text) => pasteBuffer(undefined, text, filename));
}
...
function pasteBuffer(e, text, filename) {
   ...
   mjsonrpc_db_paste([path], [odbJson]).then(rpc =>
      dlgAlert("File \"" + filename + "\" successfully loaded to ODB at " + path)).catch(error =>
      mjsonrpc_error_alert(error));
}

function make_dialog_scroll(tb) {
   let d = tb.parentElement.parentElement.parentElement.parentElement;

   if (tb.odb.picker)
      d.style.position = "absolute"; // remove "fixed"
}
   ...
   if (odb.subdirFirst) {
      for (const key of odb.key) {
         ...
         if (key.subdir_open) {
            ...
            // Propagate flags to all subkeys
            key.value.skip_yellow = odb.skip_yellow;
            key.value.subdirFirst = odb.subdirFirst;
...
function bnToHex(val) {
   let hex;
   if (val < 0) {
      hex = ((1n << 64n) + BigInt(val)).toString(16).toUpperCase();
   ...
   if (Array.isArray(key.value)) {
      ...
      if (key.type === TID_BOOL) {
         ...
         if (key.type === TID_STRING && v === "")
            ...
            v = "0x" + v.substring(2).toUpperCase();
            if (key.type === TID_UINT64)
               v += " (" + BigInt(key.value).toString() + ')';
            else
               v += " (" + parseInt(key.value) + ')';
         } else if (key.type !== TID_STRING && key.type !== TID_LINK && key.type !== TID_FLOAT && key.type !== TID_DOUBLE) {
            if (key.type === TID_INT8)
               v += " (0x" + (key.value & 0xFF).toString(16).toUpperCase() + ')';
            else if (key.type === TID_INT16)
               v += " (0x" + (key.value & 0xFFFF).toString(16).toUpperCase() + ')';
            else if (key.type === TID_INT32)
               v += " (0x" + (key.value >>> 0).toString(16).toUpperCase() + ')';
            else if (key.type === TID_INT64)
               v += " (0x" + bnToHex(BigInt(key.value)) + ')';
            else
      ...
      if (key.expanded === false) {
         ...
         if (key.type === TID_BOOL) {
            ...
            if (key.type === TID_STRING && v === "")
               ...
               v = "0x" + v.substring(2).toUpperCase();
               if (key.type === TID_UINT64)
                  v += " (" + BigInt(key.value[i]).toString() + ')';
               else
                  v += " (" + parseInt(key.value[i]) + ')';
            } else if (key.type !== TID_STRING && key.type !== TID_LINK && key.type !== TID_FLOAT && key.type !== TID_DOUBLE) {
               if (key.type === TID_INT8)
                  v += " (0x" + (key.value[i] && 0xFF).toString(16).toUpperCase() + ')';
               else if (key.type === TID_INT16)
                  v += " (0x" + (key.value[i] && 0xFFFF).toString(16).toUpperCase() + ')';
               else if (key.type === TID_INT32)
                  v += " (0x" + (key.value[i] && 0xFFFFFFFF).toString(16).toUpperCase() + ')';
               else if (key.type === TID_INT64)
                  v += " (0x" + bnToHex(BigInt(key.value[i])) + ')';
               else
                  ...
                  for (let i = 0; i < 8; i++) {
                     ...
                     if (i === 2) {
                        ...
                        if (select)
                           ...
                           if (tr.childNodes[2].childNodes[1] === undefined)
                              newValue = tr.childNodes[2].childNodes[0].data;
                           else
