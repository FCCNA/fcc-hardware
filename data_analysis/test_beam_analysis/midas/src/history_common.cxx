...
int hs_get_history(HNDLE hDB, HNDLE hKey, int flags, int debug_flag, MidasHistoryInterface **mh)
{
   ...
   *mh = NULL;
   ...
   if (strcasecmp(type.c_str(), "MIDAS")==0) {
   ...
   } else if (strcasecmp(type.c_str(), "PGSQL")==0) {

      std::string writer_dsn = "pgsql_writer.txt";
      std::string reader_dsn = "pgsql_reader.txt";

      status = db_get_value_string(hDB, hKey, "PGSQL Writer", 0, &writer_dsn, TRUE);
      assert(status == DB_SUCCESS);

      status = db_get_value_string(hDB, hKey, "PGSQL Reader", 0, &reader_dsn, TRUE);
      assert(status == DB_SUCCESS);
      ...
      int downsample = 0;
      size = sizeof(downsample);
      status = db_get_value(hDB, hKey, "Enable downsampling queries", &downsample, &size, TID_BOOL, TRUE);
      assert(status == DB_SUCCESS);

      if (active || (flags & HS_GET_INACTIVE)) {
         *mh = MakeMidasHistoryPgsql();
         if (*mh == NULL)
            return HS_FILE_ERROR;

         (*mh)->hs_set_debug(debug);
         //(*mh)->hs_set_downsample(downsample);

         status = (*mh)->hs_connect(path.c_str());
         if (status != HS_SUCCESS) {
            cm_msg(MERROR, "hs_get_history", "Cannot connect to PGSQL history, status %d", status);
            return status;
         }

         if (debug_flag)
            cm_msg(MINFO, "hs_get_history", "Connected history channel \'%s\' type PGSQL at \'%s\'", key.name,
                   path.c_str());
      }
